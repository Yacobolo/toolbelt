...
curl -fsSL https://raw.githubusercontent.com/Yacobolo/toolbelt/main/sourcebook/install.sh |
  SOURCEBOOK_VERSION=0.7.0 SOURCEBOOK_INSTALL_DIR="$HOME/bin" sh
```
...
Run `sourcebook` with no arguments to print exactly the same command overview
as `sourcebook --help`. The overview is generated by Cobra and includes every
command and flag. Use `sourcebook <command> --help` for command-specific help.
...
Sourcebook does not perform background update checks. `sourcebook upgrade` is
the only command that checks for or installs a CLI update.
...
Add documentation with an explicit catalogue preset:
...
Refresh sources by name or refresh every source explicitly:
...
`SKILL.md` names the current sources in its trigger description, includes the
exact root output of `sourcebook --help`, and keeps a concise source table of
contents. `sources.json` records source names, providers, URLs, and update
times. Existing `repos.json` installations migrate automatically after the
...
```sh
go build -ldflags "-X main.version=v0.7.0" -o sourcebook ./cmd/sourcebook
```
...
```sh
git tag sourcebook/v0.7.0
git push origin sourcebook/v0.7.0
```
