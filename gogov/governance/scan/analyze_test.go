...
func TestAnalyzeTagsIgnoredGeneratedAndTestFiles(t *testing.T) {
	t.Parallel()

	repoRoot := filepath.Join(t.TempDir(), "gitmod")
	mustWriteFile(t, filepath.Join(repoRoot, "go.mod"), "module example.com/gitmod\n\ngo 1.24\n")
	mustWriteFile(t, filepath.Join(repoRoot, ".gitignore"), "ignored/\n")
	mustWriteFile(t, filepath.Join(repoRoot, "main.go"), `package main

func main() {}
`)
	mustWriteFile(t, filepath.Join(repoRoot, "draft.go"), `package main

func draft() {}
`)
	mustWriteFile(t, filepath.Join(repoRoot, "gen", "types.gen.go"), `// Code generated by test. DO NOT EDIT.

package gen

type Example struct{}
`)
	mustWriteFile(t, filepath.Join(repoRoot, "ignored", "skip.go"), `package ignored

func skip() {}
`)
	mustWriteFile(t, filepath.Join(repoRoot, "runtime_test.go"), `package main
...
`)
	...
	runGit(t, repoRoot, "add", "go.mod", ".gitignore", "main.go", "gen/types.gen.go", "runtime_test.go")

	repo := config.Repository{
		ID:                 "gitmod",
		Name:               "gitmod",
		Root:               repoRoot,
		RuntimeDir:         filepath.Join(t.TempDir(), ".governance", "repos", "gitmod"),
		DatabasePath:       filepath.Join(t.TempDir(), ".governance", "repos", "gitmod", "governance.db"),
		LockPath:           filepath.Join(t.TempDir(), ".governance", "repos", "gitmod", "refresh.lock"),
		CoverageOutputPath: filepath.Join(t.TempDir(), ".governance", "repos", "gitmod", "coverage.out"),
	}
	...
	assertFilePresent(t, result, "main.go")
	assertFilePresent(t, result, "draft.go")
	assertTaggedFile(t, result, "ignored/skip.go", func(item model.File) bool { return item.IsIgnored })
	assertTaggedFile(t, result, "gen/types.gen.go", func(item model.File) bool { return item.IsGenerated })
	assertTaggedFile(t, result, "runtime_test.go", func(item model.File) bool { return item.IsTest })
}

func TestIsGeneratedGoFileRecognizesHeadersAndCommonSuffixes(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := isGeneratedGoFile(tc.path, []byte(tc.src))
			if got != tc.wantGen {
				t.Fatalf("isGeneratedGoFile(%q) = %v, want %v", tc.path, got, tc.wantGen)
			}
		})
	}
}
...
func assertTaggedFile(t *testing.T, result model.Snapshot, path string, predicate func(model.File) bool) {
	t.Helper()
	for _, file := range result.Files {
		if file.Path == path {
			if !predicate(file) {
				t.Fatalf("file %q did not include the expected tag state: %+v", path, file)
			}
			return
		}
	}
